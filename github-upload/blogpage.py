...
# 메모리 기반 데이터 저장 (Serverless 환경용)
posts_data = DEFAULT_POSTS.copy()
categories_data = DEFAULT_CATEGORIES.copy()

# 데이터 로드/저장 함수들 - 메모리 기반으로 수정
def load_posts():
    return posts_data
...
def martins_log():
    posts = [post for post in load_posts() if post.get('post_type') == 'log']
    posts.sort(key=lambda x: x.get('date', ''), reverse=True)
    return render_template('index.html', recent_posts=posts[:6], parenting_tips=[])

@app.route('/martins-tip')
def martins_tip():
    tips = [post for post in load_posts() if post.get('post_type') == 'tip']
    tips.sort(key=lambda x: x.get('date', ''), reverse=True)
    return render_template('index.html', recent_posts=[], parenting_tips=tips[:6])
...
def admin():
    posts = load_posts()
    posts.sort(key=lambda x: x.get('date', ''), reverse=True)
    return render_template('admin_simple.html', posts_list=posts)

@app.route('/admin/write')
@login_required
def admin_write():
    categories = load_categories()
    return render_template('admin_simple.html', active_tab='write', categories=categories)
...
def admin_posts():
    posts = load_posts()
    posts.sort(key=lambda x: x.get('date', ''), reverse=True)
    return render_template('admin_simple.html', active_tab='posts', posts_list=posts)

@app.route('/admin/settings')
@login_required
def admin_settings():
    categories = load_categories()
    return render_template('admin_simple.html', active_tab='settings', categories=categories)
